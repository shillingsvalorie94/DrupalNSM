...
function gavias_kunco_makeid($length = 5){
...
function gavias_kunco_base_url(){
  global $base_url;
  $theme_path = drupal_get_path('theme', 'gavias_kunco');
  return $base_url . '/' . $theme_path . '/';
}

function gavias_kunco_preprocess_node(&$variables) {
...
function gavias_kunco_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function gavias_kunco_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
  ...
  $variables['theme_uri'] = base_path() . drupal_get_path('theme', 'gavias_kunco');
}

function gavias_kunco_preprocess_breadcrumb(&$variables){
  ...
  if($variables['breadcrumb']){
    ...
    }elseif($node = \Drupal::routeMatch()->getParameter('commerce_product')){
      $variables['breadcrumb'][] = array(
        'text' => ''
      );
      $variables['breadcrumb'][] = array(
